...
`apple-cluster-docs` — (no description) — `bin/apple-cluster-docs`
`apple-do` — the SCRIPTABLE front door to the toolbox: pick a capability yourself, — `bin/apple-do`
...
`apple-energy` — read macOS energy/power data from the command line. — `bin/apple-energy`
...
`apple-intent` — natural language → an Apple-skill action, via on-device ML — `bin/apple-intent`
`apple-keywords` — the "useful words, no junk" extractor. Apple-native, no deps. — `bin/apple-keywords`
`apple-lang` — Apple-native language detector. — `bin/apple-lang`
`apple-lemma` — Apple-native lemma extractor. — `bin/apple-lemma`
`apple-ner` — Apple-native named-entity recognition (NaturalLanguage NLTagger, — `bin/apple-ner`
`apple-ner.swift` — apple-ner — Apple-native named-entity recognition pipe. — `bin/apple-ner.swift`
`apple-panel` — (no description) — `bin/apple-panel`
`apple-pos` — Apple-native part-of-speech tagger. — `bin/apple-pos`
...
`build-screen-app` — (no description) — `bin/build-screen-app`
...
`build-wordcloud-quickaction` — (no description) — `bin/build-wordcloud-quickaction`
...
`doi2scihub` — thin shim. The implementation now lives in convey: — `bin/doi2scihub`
...
`fa-apple-skills` — (no description) — `bin/fa-apple-skills`
...
`fa-video-analysis-mailer` — (no description) — `bin/fa-video-analysis-mailer`
...
`finances` — one word for the whole finances flow. — `bin/finances`
...
`fleet-wake` — (no description) — `bin/fleet-wake`
...
`iphone-clip` — get an iPhone photo onto the macOS clipboard, so Cmd-V pastes — `bin/iphone-clip`
...
`overlay-anchor-probe` — (no description) — `bin/overlay-anchor-probe`
`overlay-ask` — (no description) — `bin/overlay-ask`
`overlay-imagine` — (no description) — `bin/overlay-imagine`
`overlay-restart-experiment` — (no description) — `bin/overlay-restart-experiment`
...
`paketti-faq-bench` — (no description) — `bin/paketti-faq-bench`
...
`recburnredact` — the recburn family spelling of `recburn-redact`. — `bin/recburnredact`
...
`sal-siri-list-phrases.py` — (no description) — `bin/sal-siri-list-phrases.py`
...
`swiftgen` — (no description) — `bin/swiftgen`
`syncthing-nudge` — (no description) — `bin/syncthing-nudge`
...
`tagpages` — list every isolate job waiting on ONE human ear. — `bin/tagpages`
...
`vault-rag` — paragraph/section-level semantic search over a folder of notes, — `bin/vault-rag`
