...
def build_report() -> str:
    ...
    lines.extend(
        [
            ...
            f"- `macosxautomation.com` missing packages: `{len([item for item in missing_packages if 'macosxautomation.com' in item])}` dead URLs.",
            f"- `iworkautomation.com` missing videos: `{iwork_video_count}`. `PresidentsSQLiteDB.zip` recovered 2026-05-06 via DB Events Examples bundle from Sal Soghoian.",
            f"- `photosautomation.com` missing videos: `{photos_video_count}`. `installer.zip` and `Photos-to-Keynote.mp4` recovered 2026-05-06 directly from Sal Soghoian.",
            "",
            "### Missing Packages",
            "",
        ]
