...
- `/rec` — whole screen + all system audio → `~/Videos/<timestamp>.mov`
- `/rec --app Renoise` — screen + ONLY that app's audio
- `/rec --also-mic` — + your microphone on a 2nd track
- `/rec --out ~/foo.mov` — custom path
- `/rec --list` — displays + audible app names

Press **Ctrl-C** to stop and finalize. Thin wrapper over `bin/screen-audio-record --system-audio`;
`--app` overrides (single-app audio wins). Same recorder the AppleToolbox 🧰 ▸ **Record Screen & Audio**
toggle drives.
