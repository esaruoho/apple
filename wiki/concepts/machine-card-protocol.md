...
3. **Service attribution** — `machine-card` on RayMac lists the Mini's queue services inside RayMac's *own* skills. They run on the Mini. "Reachable via shared queue" is defensible, but cleaner fleet semantics attribute heartbeat-services to the machine that runs them.
