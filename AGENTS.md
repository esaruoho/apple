# Apple Repo — Codex Instructions

Project-specific instructions for sessions in `/Users/esaruoho/work/apple/`. **Skill loader is `skill.md`**; **wiki index is `wiki/INDEX.md`**; **rules from `~/.Codex/AGENTS.md`** also apply.
...
**Why (2026-06-12):** while debugging wedged FM I repeatedly `nohup`'d `fm-worker`/`fm-service` and bounded read-only probes with `nohup … & kill`. Robust wins over brittle. Companions: `~/.Codex/AGENTS.md`, `comms/AGENTS.md`, `convey/AGENTS.md`, `~/.Codex/skills/cloudcity/skill.md`.
...
- Always include `Co-Authored-By: Codex Opus 4.7 (1M context) <noreply@anthropic.com>`.
...
- Cloudcity agent at `~/work/pakettibot-agent/src/agents/cloudcity.js` runs Codex locally on the Mac Mini — no SSH needed.
