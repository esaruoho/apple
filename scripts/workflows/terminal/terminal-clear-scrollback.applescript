-- Clear scrollback in the front Terminal tab
-- App: Terminal
-- Triggers: clear, scrollback, front, terminal, tab
-- Category: Terminal
-- Usage: osascript scripts/workflows/terminal/terminal-clear-scrollback.applescript
-- Generated by workflow-gen.py

-- Concept: tell application ... end tell
--   Sends Apple Events to an app. The app must have a scripting dictionary (sdef).

tell application "Terminal"
    set contents of selected tab of front window to ""
end tell
