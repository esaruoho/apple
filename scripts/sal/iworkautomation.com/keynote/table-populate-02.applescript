...
tell application "Keynote"
	...
	-- create and poplate a table reading the data as column based
	tell document 1
		tell current slide
			if useHeaders is true then
				set thisTable to ¬
					make new table with properties ¬
						{row count:rowCount, column count:columnCount, name:"Column Data Table", header row count:1, header column count:1}
			else
				make new table with properties ¬
					{row count:rowCount, column count:columnCount, name:"Column Data Table", header row count:0, header column count:0}
			end if
			tell thisTable
				if useHeaders is true then
