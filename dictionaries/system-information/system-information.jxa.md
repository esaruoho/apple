# System Information — JavaScript for Automation (JXA) Reference

> Rendered from `system-information.yaml` by `bin/sdef-to-jxa.py`. JXA dialect of the AppleScript dictionary in `system-information.md`.
> Translation rules: WWDC 2014 #306 — *JavaScript for Automation* (Sal Soghoian + David Steinberg).

## Get the application

```javascript
var SystemInformation = Application('System Information')
SystemInformation.includeStandardAdditions = true   // if calling beep/say/displayDialog
```

**Commands:** 14  ·  **Classes:** 12  ·  **Suites:** 5
...
```javascript
// Close an object.
SystemInformation.close(target, {saving: /* savo */, savingIn: /* alias */})

// Return the number of elements of a particular class within an object.
SystemInformation.count(target, {each: /* type */})

// Delete an object.
SystemInformation.delete(target)

// Copy object(s) and put the copies at a new location.
SystemInformation.duplicate(target, {to: Path('/path'), withProperties: {}})

// Verify if an object exists.
SystemInformation.exists(target)

// Get the data for an object.
SystemInformation.get(target)

// Make a new object.
SystemInformation.make({new: /* type */, at: Path('/path'), withData: /* any */, withProperties: {}})

// Move object(s) to a new location.
SystemInformation.move(target, {to: Path('/path')})

// Open an object.
SystemInformation.open(/* alias */)

// Print an object.
SystemInformation.print(/* alias */, {printDialog: true, withProperties: /* print settings */})

// Quit an application.
SystemInformation.quit({saving: /* savo */})

// Save an object.
SystemInformation.save(target, {as: '...', in: /* alias */})

// Set an object's data.
SystemInformation.set(target, {to: /* any */})
...
## Suite — System Profiler suite

_System Profiler specific classes._

### Commands

```javascript
// Send to AppleCare
SystemInformation.send(target)

```

## Suite — Type Definitions

_Records used in scripting System Information_
