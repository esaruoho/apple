# Imovie — JavaScript for Automation (JXA) Reference

> Rendered from `imovie.yaml` by `bin/sdef-to-jxa.py`. JXA dialect of the AppleScript dictionary in `imovie.md`.
> Translation rules: WWDC 2014 #306 — *JavaScript for Automation* (Sal Soghoian + David Steinberg).

## Get the application

```javascript
var Imovie = Application('Imovie')
Imovie.includeStandardAdditions = true   // if calling beep/say/displayDialog
```
...
```javascript
// Close an object.
Imovie.close(target, {saving: /* savo */, savingIn: /* alias */})

// Return the number of elements of a particular class within an object.
Imovie.count(target, {each: /* type */})

// Delete an object.
Imovie.delete(target)

// Copy object(s) and put the copies at a new location.
Imovie.duplicate(target, {to: Path('/path'), withProperties: {}})

// Verify if an object exists.
Imovie.exists(target)

// Get the data for an object.
Imovie.get(target)

// Make a new object.
Imovie.make({new: /* type */, at: Path('/path'), withData: /* any */, withProperties: {}})

// Move object(s) to a new location.
Imovie.move(target, {to: Path('/path')})

// Open an object.
Imovie.open(/* alias */)

// Print an object.
Imovie.print(/* alias */, {printDialog: true, withProperties: /* print settings */})

// Quit an application.
Imovie.quit({saving: /* savo */})

// Save an object.
Imovie.save(target, {as: '...', in: /* alias */})

// Set an object's data.
Imovie.set(target, {to: /* any */})
...
_Records used in scripting iMovie_
