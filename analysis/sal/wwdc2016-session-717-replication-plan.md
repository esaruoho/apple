...
- **v3 (2026-05-07): multi-turn history IS READ.** The Shortcut's "Get Contents of File" action is replaced with a Run Shell Script action calling `bin/sal-siri-read-recent-turns.py` (default: last 5 turns within 30-minute staleness window). The reader emits both `PREVIOUS TURN: {...}` (single-turn deixis, v2 behavior) and `RECENT TURNS: -1: ... -2: ... -3: ...` (multi-turn deixis). System prompt has a new MULTI-TURN HISTORY section listing five common multi-turn patterns ("go back two steps", "do those last three again", "the photo I edited a minute ago", "undo that", "what did I just do"). Staleness filter verified end-to-end on local seed data.
- All four capability differences from Sal's killed prototype now reproduced AND extended: deterministic match ✅, cross-app composition ✅, slot-filling ✅, stateful conversation ✅ (single + multi-turn).
